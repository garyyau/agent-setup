...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript. Transcripts live under `~/.claude/projects/<project-slug>/<session-id>.jsonl`, with subagent transcripts under `<session-id>/subagents/`. Resolve the slug for the current working directory only; don't glob across `~/.claude/projects/*/`, which reads unrelated private chats. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill). Walk the log against what actually happened:
...
Before handing back, you must spawn a subagent on a different model from the one that did the work. Pair them off: if opus-5 did the work, review on fable-5, and vice versa. Self-review is not a substitute; the point is fresh eyes you cannot bring yourself. The subagent reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
