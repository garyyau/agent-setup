...
Distinct from reading the code and from recovering its rationale. Those tell you what the code does and why it is shaped that way. Blast radius tells you what it breaks somewhere else.
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Pull the PR and commits with `gh pr view --json title,body,files` and `git log -p`.
...
6. For a big or wide change, ask the same question of several models and merge the answers. Different models catch different real bugs. Spawn one `Agent` per model (see the model table in CLAUDE.md; fable-5 and opus-5 are the review pair) with the same brief, then reconcile: a risk only one model raises still needs proving, and agreement across models is high-signal.
