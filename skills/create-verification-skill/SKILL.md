...
The map goes stale as the app changes: features move, selectors change, launch commands drift. Offer a periodic pass that re-reads the source per feature, drives every mapped feature once against the running app, and corrects only what it proved wrong. Suggest a cadence only if they ask.

Upstream ships this as a separate `maintain-verification-skill`, which is not vendored here. Vendor it from the pin in PROVENANCE.md if the map starts drifting.
